...
const GRAD_TEXT = "bg-linear-to-r from-orange-600 to-amber-600 dark:from-orange-400 dark:to-amber-300 bg-clip-text text-transparent"
    ...
    <div className={`${PAGE_BG} animate-fadeIn relative`}>
      ...
      <div className="relative z-10">
        ...
        <div className="mb-12">
          ...
          <div className="w-16 h-1 bg-linear-to-r from-orange-500 to-amber-500 rounded-full mt-4" />
        </div>
        ...
        <div className="flex flex-wrap gap-3 mb-10">
          {categories.map((cat) => (
            <button
              ...
              className={
                activeFilter === cat
                  ? "bg-linear-to-r from-orange-500 to-amber-500 text-white rounded-full px-4 py-1.5 text-sm font-semibold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-orange-400"
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {filtered.map((proj) => (
            ...
            >
              {/* Featured badge */}
              {proj.featured && (
                <span className="absolute top-4 right-4 bg-linear-to-r from-orange-500 to-amber-500 text-white text-xs px-2.5 py-1 rounded-full font-semibold flex items-center gap-1">
                  <Star size={10} />
                  Featured
                </span>
              )}
              ...
              <div className="flex justify-between items-center mt-auto pt-4 border-t border-orange-100 dark:border-[#3d2410]">
                ...
                <Link
                  to={`/projects/${proj.id}`}
                  className={`text-xs font-semibold ${ACCENT} hover:underline underline-offset-4`}
                >
                  View Details
                </Link>
              </div>
